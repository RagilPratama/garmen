...
class BahanProsesPotongController extends Controller
{
    ...
    public function index()
    {
        ...
        $allRows = BahanProsesPotong::query()
            ->select('id', 'po', 'tanggal_potong', 'model', 'kode_bahan', 'quantity', 'satuan', 'hasil_potongan')
            ->with('bahanMasuk:kode_bahan,nama_bahan')
            ->when($search, fn($q) => $q->where(fn($q) => $q->where('po', 'like', "%{$search}%")->orWhere('model', 'like', "%{$search}%")))
        ...
        $paginator = new \Illuminate\Pagination\LengthAwarePaginator($items, $total, $perPage, $page, ['path' => request()->url(), 'query' => request()->query()]);

        $bahanOptions = BahanKeluar::selectRaw('kode_bahan, SUM(yard) as total_yard')
            ->addSelect([
                'nama_bahan' => BahanMasuk::select('nama_bahan')->whereColumn('kode_bahan', 'bahan_keluar.kode_bahan')->orderByDesc('id')->limit(1),
            ])
            ->groupBy('kode_bahan')
            ->orderBy('kode_bahan')
            ->get();
    ...
    {
        return Inertia::render('BahanProsesPotong/Form');
    }
    public function store(Request $request)
    {
        $request->validate([
            ...
            'models.*.bahans.*.quantity' => 'required|numeric|min:0',
            'models.*.bahans.*.satuan' => 'required|in:yard,meter,kg',
        ]);
    ...
    public function edit(BahanProsesPotong $bahanProsesPotong)
    {
        return Inertia::render('BahanProsesPotong/Form', ['item' => $bahanProsesPotong]);
    }
    public function update(Request $request, BahanProsesPotong $bahanProsesPotong)
    {
        ...
        $bahanProsesPotong->update($request->only(['tanggal_potong', 'quantity', 'satuan', 'po', 'model', 'kode_bahan', 'hasil_potongan']));
    ...
    public function destroy(BahanProsesPotong $bahanProsesPotong)
    {
        StokBahan::where('kode_bahan', $bahanProsesPotong->kode_bahan)->update([
            'quantity' => DB::raw("quantity + {$bahanProsesPotong->quantity}"),
        ]);
    ...
    public function updateModel(Request $request)
    {
        $request->validate([
            ...
            'bahans.*.quantity' => 'required|numeric|min:0',
            'bahans.*.satuan' => 'required|in:yard,meter,kg',
        ]);

        // Kembalikan stok dari baris lama (aggregate per kode unik)
        $oldRows = BahanProsesPotong::where('po', $request->po)
            ->where('model', $request->model)
            ->get(['kode_bahan', 'quantity']);

        $oldQuantityPerKode = [];
        foreach ($oldRows as $old) {
            $oldQuantityPerKode[$old->kode_bahan] = ($oldQuantityPerKode[$old->kode_bahan] ?? 0) + $old->quantity;
        }
        foreach ($oldQuantityPerKode as $kode => $qty) {
            StokBahan::where('kode_bahan', $kode)->update([
        ...
        BahanProsesPotong::where('po', $request->po)->where('model', $request->model)->delete();
        ...
        $newQuantityPerKode = [];
        foreach ($request->bahans as $bahan) {
            $newQuantityPerKode[$bahan['kode_bahan']] = ($newQuantityPerKode[$bahan['kode_bahan']] ?? 0) + $bahan['quantity'];
        }
        foreach ($newQuantityPerKode as $kode => $qty) {
            StokBahan::where('kode_bahan', $kode)->update([
