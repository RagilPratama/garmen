...
class KasGarmenController extends Controller
{
    public function index(Request $request)
    {
        $query = KasGarmen::with('user');
        ...
        $saldoKas = SaldoKas::where('entitas', 'garmen')->first();

        return Inertia::render('KasGarmen/Index', [
            'kasData' => $kasData,
            'saldoKas' => $saldoKas ? [
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($validated) {
            $saldoKas = SaldoKas::where('entitas', 'garmen')->lockForUpdate()->first();
        ...
        return redirect()->route('kas-garmen.index')->with('success', 'Transaksi kas garmen berhasil ditambahkan');
    }

    public function destroy(KasGarmen $kasGarmen)
    {
        DB::transaction(function () use ($kasGarmen) {
            $saldoKas = SaldoKas::where('entitas', 'garmen')->lockForUpdate()->first();
            $metodeColumn = 'saldo_' . $kasGarmen->metode_bayar;

            // Rollback saldo
            if ($kasGarmen->jenis === 'masuk') {
                $saldoKas->$metodeColumn -= $kasGarmen->jumlah;
            } else {
                $saldoKas->$metodeColumn += $kasGarmen->jumlah;
            }
            $saldoKas->save();
        ...
        return redirect()->route('kas-garmen.index')->with('success', 'Transaksi kas garmen berhasil dihapus');
    }

    public function transfer(Request $request)
    {
        $validated = $request->validate([
            'entitas_penerima' => 'required|in:gudang',
            'tanggal' => 'required|date',
            'metode_bayar' => 'required|in:cash,transfer,debit',
            'rekening_id' => 'nullable|exists:rekening,id',
        ...
        try {
            DB::transaction(function () use ($validated) {
                ...
                // Validasi saldo garmen mencukupi
                if ($saldoGarmen->$metodeColumn < $validated['jumlah']) {
                    throw new \Exception('Transfer gagal! Saldo ' . strtoupper($validated['metode_bayar']) . ' garmen tidak mencukupi. Saldo tersedia: Rp ' . number_format($saldoGarmen->$metodeColumn, 0, ',', '.'));
                }

                // Kurangi saldo garmen
                ...
                // Catat transaksi keluar di kas garmen
                KasGarmen::create([
                    'tanggal' => $validated['tanggal'],
                    'jenis' => 'keluar',
                    'metode_bayar' => $validated['metode_bayar'],
                ...
                KasGudang::create([
                    ...
                    'metode_bayar' => $validated['metode_bayar'],
                    'rekening_id' => $validated['rekening_id'] ?? null,
                    'kategori' => 'Transfer dari Garmen',
                    'jumlah' => $validated['jumlah'],
                    'saldo_sesudah' => $saldoGudang->$metodeColumn,
                    'keterangan' => $validated['keterangan'] ?? 'Transfer dari Garmen',
                    'user_id' => auth()->id(),
                ]);

                // Catat di tabel transfer
                KasTransferGudangGarmen::create([
                    'entitas_pengirim' => 'garmen',
            ...
            return redirect()->route('kas-garmen.index')->with('success', 'Transfer ke gudang berhasil');
        } catch (\Exception $e) {
