<template>
    <Teleport to="body">
        <Transition name="cd-backdrop">
            <div v-if="modelValue" class="fixed inset-0 z-[70] flex items-center justify-center p-4" @mousedown.self="$emit('update:modelValue', false)">
                <!-- Backdrop -->
                <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" />

                <!-- Panel -->
                <Transition name="cd-panel" appear>
                    <div v-if="modelValue" class="relative bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full">
                        <!-- Icon -->
                        <div class="cd-icon-wrapper relative flex items-center justify-center mx-auto mb-4">
                            ...
                            <div class="w-14 h-14 rounded-full flex items-center justify-center relative z-10" :class="variant === 'warning' ? 'bg-amber-100' : 'cd-icon-circle bg-red-100'">
                                <!-- Trash icon (danger) -->
                                <svg v-if="variant === 'danger'" class="w-7 h-7 text-red-500 cd-icon-shake" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path
                        ...
                        <div class="flex gap-3">
                            ...
                            <button
                                ...
                                :class="variant === 'warning' ? 'bg-amber-500 hover:bg-amber-600' : 'bg-red-500 hover:bg-red-600'"
                                :disabled="loading"
                                @click="$emit('confirm')"
                            >
...
.cd-backdrop-leave-active {
    transition: opacity 0.2s ease;
}
.cd-backdrop-enter-from,
.cd-backdrop-leave-to {
    opacity: 0;
}

/* Panel scale + slide */
.cd-panel-enter-active {
    transition:
        opacity 0.22s ease,
        transform 0.22s ease;
}
.cd-panel-leave-active {
    transition:
        opacity 0.15s ease,
...
@keyframes cd-pulse {
    0%,
    100% {
        box-shadow: 0 0 0 0 rgb(239 68 68 / 0.2);
    }
    50% {
