        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    <!-- Search -->
                    <div class="relative">
                        <svg v-if="!isSearching" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0" />
                        </svg>
                        <svg v-else class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-amber-500 animate-spin" fill="none" viewBox="0 0 24 24">
                            ...
                            <td v-for="col in columns" :key="col.key" class="px-5 py-3.5">
                                <slot :name="`cell-${col.key}`" :item="item" :value="item[col.key]">
                                    <template v-if="col.type === 'status'">
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold" :class="statusClass(item[col.key])">
