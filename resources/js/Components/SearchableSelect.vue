<template>
    <div class="relative" ref="wrapper">
        ...
        <button
            ...
            :class="[isOpen ? 'border-amber-400 ring-2 ring-amber-400' : 'border-gray-200', disabled ? 'opacity-50 cursor-not-allowed bg-gray-50' : 'cursor-pointer hover:border-amber-300']"
            :disabled="disabled"
        >
        ...
        <Teleport to="body">
            <div v-if="isOpen" ref="dropdownEl" class="fixed z-[9999] bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden" :style="dropdownStyle">
                <!-- Search -->
                ...
                <ul ref="listEl" class="max-h-52 overflow-y-auto py-1">
                    <!-- Empty option -->
                    <li v-if="clearable && modelValue !== null && modelValue !== ''" @click="select(null)" class="px-4 py-2 text-sm text-gray-400 hover:bg-gray-50 cursor-pointer italic">
                        — Hapus pilihan —
                    </li>
                    <li
                        v-if="allowCustom && query.trim() && !normalised.find((o) => o.value === query.trim())"
                    ...
                    <li v-if="filtered.length === 0 && !(allowCustom && query.trim())" class="px-4 py-2 text-sm text-gray-400 text-center py-4">Tidak ada hasil</li>
                    <li
                        v-for="(opt, idx) in filtered"
                        :key="opt.value"
                        @click="select(opt.value)"
                        class="px-4 py-2 text-sm cursor-pointer transition-colors flex items-center gap-2"
                        :class="[opt.value === modelValue ? 'bg-amber-50 text-amber-700 font-medium' : 'text-gray-700 hover:bg-gray-50', focusedIndex === idx ? 'bg-gray-100' : '']"
                    >
