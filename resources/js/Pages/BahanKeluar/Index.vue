<template>
    <DataTable title="Bahan Keluar ke Garment" :data="data" :columns="columns" base-path="/bahan-keluar" @open-create="openCreate" @open-edit="openEdit">
        <!-- No. Surat Jalan clickable -->
        <template #cell-no_surat_jalan="{ item }">
            <button type="button" @click="openDetail(item)" class="font-semibold text-amber-600 hover:text-amber-800 hover:underline transition-colors">
                {{ item.no_surat_jalan ?? '—' }}
            </button>
        </template>
        ...
        <template #cell-items_count="{ item }">
            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-700">{{ item.items_count }} item</span>
        </template>
            ...
            <Modal v-model="showCreateModal" title="Tambah Bahan Keluar" size="xl">
                <form @submit.prevent="submitCreate" class="space-y-5">
                    ...
                    <div>
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    <tr v-for="(item, idx) in createForm.items" :key="idx">
                                        ...
                                        <td class="px-2 py-2 text-center">
                                            <button
                                                type="button"
                                                @click="createForm.items.splice(idx, 1)"
                        ...
                        <button
                            type="button"
                            @click="createForm.items.push({ kode_bahan: '', yard: '', rp_per_yard: '' })"
            ...
            <Modal v-model="showEditModal" title="Edit Bahan Keluar" size="xl">
                <form @submit.prevent="submitEdit" class="space-y-5">
                    ...
                    <div>
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    <tr v-for="(item, idx) in editForm.items" :key="idx">
                                        <td class="px-3 py-2 text-gray-400 text-xs">{{ idx + 1 }}</td>
                                        <td class="px-3 py-2">
                                            <SearchableSelect v-model="item.kode_bahan" :options="editBahanOptions" placeholder="-- Pilih --" />
                        ...
                        <button
                            type="button"
                            @click="editForm.items.push({ kode_bahan: '', yard: '', rp_per_yard: '' })"
                            class="mt-2 text-sm text-amber-600 hover:text-amber-700 font-medium flex items-center gap-1"
                        >
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                            Tambah Bahan
            ...
            <Modal v-model="showDetailModal" title="Detail Surat Jalan" size="lg">
                <div v-if="detailNota" class="space-y-4">
                    ...
                    <div class="flex justify-end gap-2 pt-2 border-t border-gray-100">
                        <button type="button" @click="openEditFromDetail" class="px-4 py-2 text-sm text-amber-700 border border-amber-300 rounded-lg hover:bg-amber-50 transition-colors">Edit</button>
                        <button type="button" @click="showDetailModal = false" class="px-4 py-2 text-sm text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                            Tutup
                        </button>
                    </div>
                </div>
