<template>
    <DataTable title="Bahan Masuk" :data="data" :columns="columns" base-path="/bahan-masuk" @open-create="openCreate" @open-edit="openEdit">
        <!-- Clickable no_nota cell -->
        <template #cell-no_nota="{ item, value }">
            <button @click.stop="openDetail(item)" class="text-amber-600 hover:text-amber-700 font-semibold hover:underline underline-offset-2 transition">
                {{ value ?? '—' }}
            </button>
        </template>
        ...
        <template #cell-sisa_tagihan="{ item }">
            <span v-if="(item.sisa_tagihan ?? item.grand_total) <= 0" class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700">
        ...
        <template #actions="{ item }">
            <button
                v-if="(item.sisa_tagihan ?? item.grand_total) > 0"
                ...
                <form v-if="editNota" @submit.prevent="submitEdit" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input
                                v-model="editForm.no_surat_jalan"
                                type="text"
                                class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition bg-white"
                        ...
                        <div>
                            ...
                            <SearchableSelect v-model="editForm.supplier" :options="supplierOptions" placeholder="-- Pilih supplier --" />
                    ...
                    <div v-if="editForm.supplier && editBahanOptions.length" class="space-y-2">
                        <p class="text-xs font-medium text-gray-500">
                            Bahan pernah dipesan dari
                            <span class="text-amber-600 font-semibold">{{ editForm.supplier }}</span>
                            — klik untuk tambah:
                        </p>
                        <div class="flex flex-wrap gap-2">
                            <button
                                v-for="opt in editBahanOptions"
                                :key="opt.kode_bahan"
                                type="button"
                            ...
                            >
                                ...
                                {{ opt.kode_bahan }}
                                <span v-if="opt.nama_bahan" class="text-amber-500">— {{ opt.nama_bahan }}</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="border border-gray-200 rounded-lg overflow-hidden">
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    <tr v-for="(item, idx) in editForm.items" :key="idx">
                                        ...
                                        <td class="px-2 py-1.5">
                                            <SearchableSelect v-model="item.nama_bahan" :options="bahanOptions" placeholder="-- Pilih Nama Bahan --" searchPlaceholder="Cari nama bahan..." />
                                        </td>
                                        <td class="px-2 py-1.5">
                                            <input
                                                v-model="item.yard"
                            ...
                            <div class="px-3 py-2 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
                                <button
                                    type="button"
                                    @click="editForm.items.push({ kode_bahan: '', nama_bahan: '', yard: '', rp_per_yard: '' })"
                    ...
                    <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                        <button type="button" @click="showEditModal = false" class="px-5 py-2.5 text-sm text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                            Batal
                        </button>
                        <button
                            type="submit"
                ...
                <form @submit.prevent="submitCreate" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input
                                    v-model="createForm.no_surat_jalan"
                                    type="text"
                        ...
                        <div>
                            ...
                            <SearchableSelect v-model="createForm.supplier" :options="supplierOptions" placeholder="-- Pilih supplier --" />
                    ...
                    <div v-if="createForm.supplier && createBahanOptions.length" class="space-y-2">
                        <p class="text-xs font-medium text-gray-500">
                            Bahan pernah dipesan dari
                            <span class="text-amber-600 font-semibold">{{ createForm.supplier }}</span>
                            — klik untuk tambah:
                        </p>
                        <div class="flex flex-wrap gap-2">
                            <button
                                v-for="opt in createBahanOptions"
                                :key="opt.kode_bahan"
                                type="button"
                    ...
                    <div>
                        ...
                        <div class="border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <div class="px-3 py-2 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
                                <button type="button" @click="addItem" class="text-sm text-amber-600 hover:text-amber-700 font-medium flex items-center gap-1 transition">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                                    </svg>
                                    Tambah Bahan
                    ...
                    <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                        <button type="button" @click="showCreateModal = false" class="px-5 py-2.5 text-sm text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                            Batal
                        </button>
                        <button
                            type="submit"
                ...
                <div v-if="selectedNota" class="space-y-4">
                    ...
                    <div class="border border-gray-200 rounded-lg overflow-hidden">
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr v-for="item in selectedNota.items" :key="item.id" class="hover:bg-amber-50/30 transition-colors">
                    ...
                    <div class="flex justify-between pt-1">
                        <div class="flex items-center gap-2">
                            ...
                            <button @click="printSuratJalan(selectedNota)" class="px-5 py-2.5 text-sm text-white bg-blue-500 hover:bg-blue-600 rounded-lg transition-colors flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path
                                        stroke-linecap="round"
                                        stroke-linejoin="round"
            ...
            <Modal v-model="showPayModal" :title="`Bayar — ${selectedNota?.no_nota ?? ''}`">
                <form v-if="selectedNota" @submit.prevent="submitPay" class="space-y-4">
                    ...
                    <div>
                        ...
                        <p v-else-if="payForm.jumlah && selectedNota && Number(payForm.jumlah) > (selectedNota.sisa_tagihan ?? selectedNota.grand_total)" class="mt-1 text-xs text-red-500">
                            Melebihi sisa tagihan ({{ formatRupiah(selectedNota.sisa_tagihan ?? selectedNota.grand_total) }})
                        </p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                    ...
                    <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                        <button type="button" @click="showPayModal = false" class="px-5 py-2.5 text-sm text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                            Batal
                        </button>
                        <button
                            type="submit"
                            :disabled="payForm.processing || (payForm.jumlah && selectedNota && Number(payForm.jumlah) > (selectedNota.sisa_tagihan ?? selectedNota.grand_total))"
                        ...
                        >
                            <svg v-if="payForm.processing" class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                            </svg>
                            Simpan Pembayaran
...
const printSuratJalan = (nota) => {
    ...
    const tanggal = nota.tanggal ? new Date(nota.tanggal).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }) : '—';
