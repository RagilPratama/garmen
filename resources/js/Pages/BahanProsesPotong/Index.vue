                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="px-5 py-3.5 text-center">
                                <span class="inline-flex items-center justify-center px-2.5 py-1 bg-emerald-50 text-emerald-700 text-xs font-semibold rounded-full">
                                    {{ group.total_hasil_potongan.toLocaleString('id-ID') }} pcs
                                </span>
                            </td>
            ...
            <div v-if="detailGroup" class="space-y-4">
                <div class="flex items-center gap-4 text-sm text-gray-600 bg-gray-50 rounded-lg px-4 py-3">
                    <span>
                        ...
                        {{ formatDate(detailGroup.tanggal_potong) }}
                    </span>
                    <span>
                    ...
                    <span>
                        ...
                        {{ detailGroup.total_hasil_potongan.toLocaleString('id-ID') }} pcs
                    </span>
                </div>
                ...
                <div v-for="mRow in detailGroup.models" :key="mRow.model" class="border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <div class="flex items-center justify-between px-4 py-2.5 bg-amber-50 border-b border-amber-100">
                        ...
                        <button @click="openEdit(mRow)" class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors">
                        ...
                        <tbody>
                            <tr v-for="bahan in mRow.bahans" :key="bahan.id" class="border-t border-gray-100 hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2 text-center">
                                    <button @click="confirmDelete(bahan.id)" class="text-gray-300 hover:text-red-400 transition">
                    ...
                    <div v-for="(mRow, mIdx) in form.models" :key="mIdx" class="border border-gray-200 rounded-lg p-3 space-y-3 bg-gray-50/50">
                        <div class="flex items-start gap-3">
                            ...
                            <button
                                ...
                                class="mt-5 text-gray-300 hover:text-red-400 transition disabled:opacity-30 disabled:cursor-not-allowed flex-shrink-0"
                            >
                        ...
                        <div class="pl-2 border-l-2 border-amber-200 space-y-2">
                            ...
                            <div v-for="(bahan, bIdx) in mRow.bahans" :key="bIdx" class="flex items-center gap-2">
                                <div class="flex-1">
                                    <SearchableSelect
                                        v-model="bahan.kode_bahan"
                                        :options="
                                            bahanOptions.map((b) => ({ value: b.kode_bahan, label: b.kode_bahan + (b.nama_bahan ? ' – ' + b.nama_bahan : '') + ' (' + fmt(b.total_yard) + ' yard)' }))
                                        "
                                ...
                                <button
                                    type="button"
                                    @click="removeBahan(mIdx, bIdx)"
                                    :disabled="mRow.bahans.length === 1"
            ...
            <form @submit.prevent="submitEdit" class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input
                            v-model="editForm.tanggal_potong"
                            type="date"
                            required
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition bg-white"
                        />
                    ...
                    <div>
                        ...
                        <input
                            v-model="editForm.hasil_potongan"
                            type="number"
                            min="0"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition bg-white"
                            placeholder="0"
                ...
                <div class="pl-2 border-l-2 border-amber-200 space-y-2">
                    ...
                    <div v-for="(bahan, bIdx) in editForm.bahans" :key="bIdx" class="flex items-center gap-2">
                        ...
                        <button
                            type="button"
                            @click="editRemoveBahan(bIdx)"
                            :disabled="editForm.bahans.length === 1"
                            class="text-gray-300 hover:text-red-400 transition disabled:opacity-30 disabled:cursor-not-allowed flex-shrink-0"
                        >
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    ...
                    <button type="button" @click="editAddBahan" class="flex items-center gap-1 text-xs text-amber-600 hover:text-amber-700 font-medium transition">
...
const removeModel = (mIdx) => {
    if (form.models.length > 1) form.models.splice(mIdx, 1);
};
const addBahan = (mIdx) => form.models[mIdx].bahans.push(emptyBahan());
const removeBahan = (mIdx, bIdx) => {
    if (form.models[mIdx].bahans.length > 1) form.models[mIdx].bahans.splice(bIdx, 1);
};
...
const editRemoveBahan = (bIdx) => {
    if (editForm.bahans.length > 1) editForm.bahans.splice(bIdx, 1);
};
const submitEdit = () => {
