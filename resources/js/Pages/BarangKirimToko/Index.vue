          ...
          <tbody>
            <tr v-for="(group, index) in data?.data" :key="group.po"
              ...
              <td class="px-5 py-3.5">
                <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 text-xs font-medium text-gray-500 group-hover:bg-indigo-100 group-hover:text-indigo-700 transition-colors">
              ...
              <td class="px-5 py-3.5 text-center">
                <span class="inline-flex items-center justify-center px-2.5 py-1 bg-indigo-50 text-indigo-700 text-xs font-semibold rounded-full">{{ group.total_pcs.toLocaleString('id-ID') }} pcs</span>
              </td>
      ...
      <div v-if="data?.last_page > 1" class="px-6 py-4 bg-gray-50/50 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-1 flex-wrap justify-center">
          <Link v-for="link in data.links" :key="link.label"
            ...
            :class="link.active ? 'bg-indigo-500 text-white font-semibold shadow-sm' : link.url ? 'text-gray-600 hover:bg-white hover:shadow-sm border border-transparent hover:border-gray-200' : 'text-gray-300 cursor-default pointer-events-none'"
      ...
      <form @submit.prevent="submit" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <div class="relative">
              <input v-model="noSuratJalan" type="text"
          ...
          <div>
            ...
            <input v-model="tanggalKirim" type="date" required
        ...
        <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
          ...
          <button type="submit" :disabled="processing || !checkedItems.size" class="px-5 py-2.5 text-sm text-white bg-indigo-500 hover:bg-indigo-600 rounded-lg disabled:opacity-60 flex items-center gap-2">
    ...
    <Modal v-model="showEditModal" title="Edit Pcs Kirim" size="sm">
      <form @submit.prevent="submitEdit" class="space-y-4">
        ...
        <div>
          ...
          <input v-model="editForm.pcs_barang_jadi" type="number" min="1" required
            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition bg-white"
            placeholder="0"/>
        ...
        <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
          ...
          <button type="submit" :disabled="editForm.processing" class="px-5 py-2.5 text-sm text-white bg-indigo-500 hover:bg-indigo-600 rounded-lg disabled:opacity-60 flex items-center gap-2">
