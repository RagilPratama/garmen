<template>
    <AdminLayout title="Barang Kirim Toko">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="h-1 bg-gradient-to-r from-indigo-400 via-blue-500 to-indigo-400"></div>
            ...
            <div class="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-base font-semibold text-gray-800">Barang Kirim Toko</h2>
                ...
                <div class="flex items-center gap-2">
                    <div class="relative">
                        ...
                        <input
                            v-model="searchQuery"
                            type="text"
                            placeholder="Cari PO / model..."
                            class="pl-9 pr-8 py-2 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent w-52 transition-all"
                        />
                        <button
                            v-if="searchQuery"
                            @click="searchQuery = ''"
                    ...
                    <button
                        @click="openCreate"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-500 to-blue-500 hover:from-indigo-600 hover:to-blue-600 text-white text-sm font-medium rounded-xl transition-all shadow-sm hover:shadow-md whitespace-nowrap"
                    >
                    ...
                    <tbody>
                        ...
                        >
                            <td class="px-5 py-3.5">
                                <span
                                    class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 text-xs font-medium text-gray-500 group-hover:bg-indigo-100 group-hover:text-indigo-700 transition-colors"
                                >
                            ...
                            <td class="px-5 py-3.5">
                                <span
                                    v-if="group.no_surat_jalan"
                                    class="inline-flex items-center gap-1 text-xs font-mono bg-indigo-50 text-indigo-700 border border-indigo-100 px-2 py-0.5 rounded font-semibold"
                                >
                            ...
                            <td class="px-5 py-3.5 text-center">
                                <span class="inline-flex items-center justify-center px-2.5 py-1 bg-indigo-50 text-indigo-700 text-xs font-semibold rounded-full">
                                    {{ group.total_pcs.toLocaleString('id-ID') }} pcs
                                </span>
                            </td>
        ...
        <Modal v-model="showDetail" :title="'Detail Pengiriman: ' + (detailGroup?.no_surat_jalan ?? '')" size="xl">
            <div v-if="detailGroup" class="space-y-4">
                <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600 bg-gray-50 rounded-lg px-4 py-3">
                    <span>
                        <span class="font-medium text-gray-700">Tgl Kirim:</span>
                        {{ formatDate(detailGroup.tanggal_kirim) }}
                    </span>
                    <span>
                    ...
                    <span>
                        <span class="font-medium text-gray-700">Jumlah Model:</span>
                        {{ detailGroup.jumlah_model }}
                    </span>
                    <span>
                        <span class="font-medium text-gray-700">Total Pcs:</span>
                        {{ detailGroup.total_pcs }} pcs
                    </span>
                </div>
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="mRow in detailGroup.models" :key="mRow.id" class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2.5 text-center">
                                <span class="text-indigo-700 font-semibold">{{ mRow.pcs_barang_jadi }}</span>
                            </td>
                            <td class="px-4 py-2.5 text-center flex items-center justify-center gap-2">
        ...
        <Modal v-model="showModal" title="Tambah Barang Kirim Toko" size="xl">
            <div v-if="!stokOptions.length" class="mb-4 flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
                ...
                Belum ada barang siap kirim dari
                <strong class="ml-1">Proses Finishing</strong>
                .
            </div>
            <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                type="text"
                                class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition bg-white pr-16"
                                placeholder="SJ-TOKO-001"
                            />
                            <span class="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded font-medium">Auto</span>
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                ...
                <div>
                    ...
                    <div class="max-h-72 overflow-y-auto border border-gray-200 rounded-xl divide-y divide-gray-100">
                        ...
                        <div v-if="!filteredOptions.length" class="px-4 py-8 text-center text-sm text-gray-400">Tidak ada stok barang siap kirim</div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                    ...
                    <button
                        ...
                        class="px-5 py-2.5 text-sm text-white bg-indigo-500 hover:bg-indigo-600 rounded-lg disabled:opacity-60 flex items-center gap-2"
                    >
        ...
        <!-- EDIT MODAL -->
        <Modal v-model="showEditModal" title="Edit Pcs Kirim" size="sm">
            <form @submit.prevent="submitEdit" class="space-y-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="0"
                    />
                    <p v-if="editForm.errors?.pcs_barang_jadi" class="mt-1 text-xs text-red-500">{{ editForm.errors.pcs_barang_jadi }}</p>
                </div>
                <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
...
watch(searchQuery, (val) => {
    ...
    searchTimer = setTimeout(() => {
        router.get('/barang-kirim-toko', { search: val || undefined }, { preserveState: true, replace: true });
    }, 350);
});
...
const totalPcs = computed(() => [...checkedItems.value].reduce((sum, key) => sum + (parseInt(pcsPerItem.value[key]) || 0), 0));
const openCreate = () => {
    tokoId.value = null;
...
const submitEdit = () => {
    editForm.put(`/barang-kirim-toko/${editRow.value.id}`, {
        onSuccess: () => {
...
const doDelete = () => {
    ...
    router.delete(`/barang-kirim-toko/${deleteId.value}`, {
        onSuccess: () => {
            deleteId.value = null;
