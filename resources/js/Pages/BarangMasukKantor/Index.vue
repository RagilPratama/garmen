<template>
  <DataTable title="Barang Masuk Kantor" :data="data" :columns="columns" base-path="/barang-masuk-kantor"
    @open-create="openCreate" @open-edit="openEdit">
    <template #modal>
      <Modal v-model="showModal" :title="editItem ? 'Edit Barang Masuk Kantor' : 'Tambah Barang Masuk Kantor'" size="lg">
        ...
        <form @submit.prevent="submit" class="space-y-4">
          <div>
            ...
            <select v-model="selectedKey" @change="onSelect" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition bg-white">
              <option value="" disabled>-- Pilih PO dari Proses Finishing --</option>
              <option v-for="o in poOptions" :key="o.po+'||'+o.model" :value="o.po+'||'+o.model">
                PO: {{ o.po }} | Model: {{ o.model }} ({{ o.max_pcs }} pcs barang jadi)
              </option>
            </select>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="form.no_surat_jalan" type="text" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition bg-white" placeholder="SJ-001"/>
            ...
            <div class="col-span-2">
              ...
              <p v-if="selectedOpt" class="mt-1 text-xs text-gray-500">Maks dari finishing: <span class="font-medium text-amber-600">{{ selectedOpt.max_pcs }} pcs</span></p>
            </div>
          </div>
...
const submit = () => {
  if (editItem.value) {
    form.put(`/barang-masuk-kantor/${editItem.value.id}`, { onSuccess: () => { showModal.value = false } })
  } else {
    form.post('/barang-masuk-kantor', { onSuccess: () => { showModal.value = false; form.reset(); selectedKey.value = '' } })
  }
}
</script>
