<template>
  <AdminLayout title="Barang Masuk Kantor">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="h-1 bg-gradient-to-r from-teal-400 via-cyan-500 to-teal-400"></div>
      ...
      <div class="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-xl bg-teal-50 flex items-center justify-center">
            <svg class="w-5 h-5 text-teal-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-2 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
          </div>
          <div>
            <h2 class="text-base font-semibold text-gray-800">Barang Masuk Kantor</h2>
          ...
          <tbody>
            <tr v-for="(group, index) in data?.data" :key="group.po"
              ...
              <td class="px-5 py-3.5">
                <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 text-xs font-medium text-gray-500 group-hover:bg-teal-100 group-hover:text-teal-700 transition-colors">
                  {{ (data.current_page - 1) * data.per_page + index + 1 }}
                </span>
              </td>
              <td class="px-5 py-3.5">
                <span class="font-semibold text-teal-600">{{ group.po }}</span>
              </td>
              ...
              <td class="px-5 py-3.5">
                <span v-if="group.no_surat_jalan" class="inline-flex items-center gap-1 text-xs font-mono bg-teal-50 text-teal-700 border border-teal-100 px-2 py-0.5 rounded">
              ...
              <td class="px-5 py-3.5 text-center">
                <span class="inline-flex items-center justify-center px-2.5 py-1 bg-teal-50 text-teal-700 text-xs font-semibold rounded-full">{{ group.total_pcs.toLocaleString('id-ID') }} pcs</span>
              </td>
              ...
              <td colspan="7" class="px-4 py-16 text-center">
                <div class="flex flex-col items-center gap-3">
                  <div class="w-16 h-16 rounded-2xl bg-gray-100 flex items-center justify-center">
                    <svg class="w-8 h-8 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-2 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                  </div>
      ...
      <div v-if="data?.last_page > 1" class="px-6 py-4 bg-gray-50/50 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-1 flex-wrap justify-center">
          <Link v-for="link in data.links" :key="link.label"
            ...
            :class="link.active ? 'bg-teal-500 text-white font-semibold shadow-sm' : link.url ? 'text-gray-600 hover:bg-white hover:shadow-sm border border-transparent hover:border-gray-200' : 'text-gray-300 cursor-default pointer-events-none'"
      ...
      <div v-if="detailGroup" class="space-y-4">
        <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600 bg-gray-50 rounded-lg px-4 py-3">
          ...
          <span><span class="font-medium text-gray-700">No. SJ:</span>
            <span class="ml-1 font-mono text-teal-600">{{ detailGroup.no_surat_jalan || '—' }}</span>
          </span>
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="mRow in detailGroup.models" :key="mRow.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-2.5 text-center">
                <span class="text-teal-700 font-semibold">{{ mRow.pcs_barang_jadi }}</span>
              </td>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <div class="relative">
              ...
              <span class="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs bg-teal-100 text-teal-700 px-1.5 py-0.5 rounded font-medium">Auto</span>
            </div>
          </div>
          <div>
    ...
    <Modal v-model="showEditModal" title="Edit Pcs Barang Jadi" size="sm">
      <form @submit.prevent="submitEdit" class="space-y-4">
        ...
        <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
          ...
          <button type="submit" :disabled="editForm.processing" class="px-5 py-2.5 text-sm text-white bg-teal-500 hover:bg-teal-600 rounded-lg disabled:opacity-60 flex items-center gap-2">
...
watch(searchQuery, (val) => {
  ...
  searchTimer = setTimeout(() => {
    router.get('/barang-masuk-kantor', { search: val || undefined }, { preserveState: true, replace: true })
  }, 350)
})
