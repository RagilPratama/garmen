        ...
        <div class="space-y-6">
            ...
            <div v-if="isAdmin" class="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:grid-cols-4">
                ...
                <div class="bg-gradient-to-br from-emerald-400 to-emerald-500 rounded-xl p-5 text-white shadow-md overflow-hidden">
                    ...
                    <p class="text-xl font-bold mt-1 leading-tight truncate" :title="formatRupiah(omsetTokoBulanIni + omsetGudangBulanIni)">
                        {{ formatRupiah(omsetTokoBulanIni + omsetGudangBulanIni) }}
                    </p>
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="bg-gradient-to-br from-emerald-400 to-emerald-500 rounded-xl p-5 text-white shadow-md overflow-hidden">
                    ...
                    <p class="text-2xl font-bold mt-1 leading-tight truncate" :title="formatRupiah(omsetTokoBulanIni - pengeluaranTokoBulanIni)">
                        {{ formatRupiah(omsetTokoBulanIni - pengeluaranTokoBulanIni) }}
                    </p>
            ...
            <div v-if="isAdmin && (sisaHutang > 0 || sisaPiutang > 0)" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div v-if="sisaPiutang > 0" class="bg-orange-50 border border-orange-200 rounded-xl px-5 py-4 flex items-center gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-orange-500 mt-0.5 truncate">
                            {{ jumlahNotaPiutang }} nota belum lunas &mdash;
                            <a href="/jual-gudang" class="underline hover:text-orange-700">Gudang</a>
                            &middot;
            ...
            <!-- Kas Section: Saldo Toko (kiri) + Gudang & Garmen (kanan atas-bawah) -->
            <div v-if="isAdmin" class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                <!-- Saldo Kas Toko (kiri) -->
                <div v-if="saldoKasToko && saldoKasToko.length > 0" class="bg-white rounded-xl border border-gray-100 shadow-sm">
                    <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                        <h3 class="font-semibold text-gray-800">Saldo Kas Toko</h3>
                        <Link href="/kas-toko" class="text-green-500 hover:text-green-600 text-xs font-medium">Kelola Kas →</Link>
                    </div>
                    <div class="p-5 space-y-4">
                        <div v-for="kas in saldoKasToko" :key="kas.kode_toko" class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-4 border border-green-100">
                            <div class="flex items-center justify-between mb-3">
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-medium text-green-600 mb-1">{{ kas.nama_toko }}</p>
                                    <p class="text-2xl font-bold text-green-700 truncate">{{ formatRupiah(kas.saldo) }}</p>
                        ...
                        <!-- Total Gabungan Semua Toko -->
                        <div v-if="saldoKasToko.length > 1" class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-xs font-medium text-gray-600 mb-1">Total Gabungan Semua Toko</p>
                                    <p class="text-xl font-bold text-gray-800">{{ formatRupiah(saldoKasToko.reduce((s, k) => s + Number(k.saldo || 0), 0)) }}</p>
                                </div>
                                <div class="text-right text-xs space-y-1">
                                    <p class="text-green-600 font-medium">Cash: {{ formatRupiah(saldoKasToko.reduce((s, k) => s + Number(k.saldo_cash || 0), 0)) }}</p>
                                    <p class="text-blue-600 font-medium">Transfer: {{ formatRupiah(saldoKasToko.reduce((s, k) => s + Number(k.saldo_transfer || 0), 0)) }}</p>
                                    <p class="text-purple-600 font-medium">Debit: {{ formatRupiah(saldoKasToko.reduce((s, k) => s + Number(k.saldo_debit || 0), 0)) }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Kas Gudang & Garmen (kanan, atas-bawah) -->
                <div class="space-y-5">
                    <div v-if="saldoKasGudang" class="bg-white rounded-xl border border-gray-100 shadow-sm">
                        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                            <h3 class="font-semibold text-gray-800">Kas Gudang</h3>
                        ...
                        <div class="p-5">
                            <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg p-4 border border-blue-100">
                                <div class="flex items-center justify-between mb-3">
                                    <div class="flex-1 min-w-0">
                                        <p class="text-xs font-medium text-blue-600 mb-1">Gudang (Kantor)</p>
                                        <p class="text-2xl font-bold text-blue-700 truncate">
                                        ...
                                        <p class="text-xs text-blue-500 mt-1">Total Saldo Kas</p>
                                    </div>
                    ...
                    <div v-if="saldoKasGarmen" class="bg-white rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div class="p-5">
                            <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg p-4 border border-orange-100">
                                <div class="flex items-center justify-between mb-3">
                                    <div class="flex-1 min-w-0">
                                        <p class="text-xs font-medium text-orange-600 mb-1">Garmen</p>
                                        <p class="text-2xl font-bold text-orange-700 truncate">
                                        ...
                                        <p class="text-xs text-orange-500 mt-1">Total Saldo Kas</p>
                                    </div>
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="font-semibold text-gray-800">
                        Model Terlaris
                        <span class="text-xs font-normal text-gray-400 ml-1">(all time, berdasarkan pcs terjual)</span>
                    </h3>
                ...
                <div v-else class="space-y-3">
                    <div v-for="(item, i) in topModels" :key="item.model" class="flex items-center gap-3">
                        <!-- Rank badge -->
                        <div
                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="text-left px-5 py-2.5 text-xs text-gray-400 font-medium">Nama Bahan</th>
                                <th class="text-left px-5 py-2.5 text-xs text-gray-400 font-medium">Supplier</th>
                                <th class="text-right px-5 py-2.5 text-xs text-gray-400 font-medium">Qty</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50">
                            <tr v-for="item in recentBahanMasuk" :key="item.id" class="hover:bg-gray-50/60">
                                <td class="px-5 py-3 font-mono text-gray-800 font-medium">{{ item.kode_bahan }}</td>
                                <td class="px-5 py-3 text-gray-600">{{ item.nama_bahan }}</td>
                                <td class="px-5 py-3 text-gray-600">{{ item.supplier }}</td>
                                <td class="px-5 py-3 text-right text-gray-700">{{ item.quantity }} yard</td>
                            </tr>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                <tr v-for="item in recentPenjualan" :key="item.channel + item.id" class="hover:bg-gray-50/60">
                                    ...
                                    <td class="px-2 py-2.5 text-center">
                                        <span
                                            class="px-1.5 py-0.5 rounded-full text-xs font-medium whitespace-nowrap"
