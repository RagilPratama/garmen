<template>
    <AdminLayout title="Jual dari Gudang">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="h-1 bg-gradient-to-r from-amber-400 via-orange-400 to-amber-400"></div>
            ...
            <div class="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-base font-semibold text-gray-800">Jual dari Gudang</h2>
                ...
                <div class="flex items-center gap-2">
                    <div class="relative">
                        ...
                        <input
                            v-model="searchQuery"
                            type="text"
                            placeholder="Cari nota / buyer / model..."
                            class="pl-9 pr-8 py-2 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent w-56 transition-all"
                        />
                        <button
                            v-if="searchQuery"
                            @click="searchQuery = ''"
                    ...
                    <button
                        @click="openCreate"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white text-sm font-medium rounded-xl transition-all shadow-sm hover:shadow-md whitespace-nowrap"
                    >
                    ...
                    <tbody>
                        ...
                        >
                            <td class="px-5 py-3.5">
                                <span
                                    class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 text-xs font-medium text-gray-500 group-hover:bg-amber-100 group-hover:text-amber-700 transition-colors"
                                >
                                    {{ (data.current_page - 1) * data.per_page + index + 1 }}
                                </span>
                            </td>
                            <td class="px-5 py-3.5">
                                <span class="font-mono text-xs bg-amber-50 text-amber-700 border border-amber-100 px-2 py-0.5 rounded">{{ group.no_nota || '—' }}</span>
                            </td>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="p in detailGroup.pembayarans" :key="p.id" class="hover:bg-gray-50/50">
                                ...
                                <td class="px-3 py-2">
                                    <span
                                        :class="p.metode === 'cash' ? 'bg-green-100 text-green-700' : p.metode === 'transfer' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'"
                                ...
                                <td class="px-3 py-2 text-center">
                                    <button @click="deletePembayaran(p.id)" type="button" class="text-red-400 hover:text-red-600 transition" title="Hapus riwayat">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path
                                                stroke-linecap="round"
                                                stroke-linejoin="round"
        ...
        <Modal v-model="showPayModal" :title="'Bayar — ' + (detailGroup?.no_nota ?? '')">
            <form v-if="detailGroup" @submit.prevent="submitPembayaran" class="space-y-4">
                ...
                <div>
                    ...
                    <input
                        v-model.number="payForm.jumlah"
                        type="number"
                        min="1"
                        :max="detailGroup.sisa_piutang"
                        required
                        placeholder="0"
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition bg-white"
                    />
                ...
                <div v-if="payForm.metode === 'transfer'">
                    ...
                    <div v-if="!rekeningOptions.length" class="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                        ...
                        Belum ada data master rekening. Tambahkan di menu
                        <a href="/rekening" class="font-semibold underline">Master Rekening</a>
                        .
                    </div>
                    <select
                        v-else
                    ...
                    >
                        ...
                        <option v-for="r in rekeningOptions" :key="r.id" :value="r.id">{{ r.bank }} — {{ r.nama }}{{ r.nomor_rekening ? ' (' + r.nomor_rekening + ')' : '' }}</option>
                    </select>
                </div>
                <div>
        ...
        <!-- CREATE MODAL -->
        <Modal v-model="showModal" title="Tambah Jual Gudang" size="xl">
            ...
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div v-if="nominalBayar > 0" class="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-200">
                    <div>
                        ...
                        <div class="flex gap-4 mt-2">
                            <label v-for="m in ['cash', 'transfer', 'debit']" :key="m" class="flex items-center gap-2 cursor-pointer">
                                <input v-model="metodeBayar" type="radio" :value="m" class="text-amber-500" />
                                <span class="text-sm text-gray-700 capitalize">{{ m }}</span>
                            </label>
                        </div>
                    </div>
                    <div v-if="metodeBayar === 'transfer'">
                        ...
                        >
                            <option value="" disabled>-- Pilih rekening --</option>
                            <option v-for="r in rekeningOptions" :key="r.id" :value="r.id">{{ r.bank }} — {{ r.nama }}</option>
                        </select>
                    </div>
                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Keterangan Bayar</label>
                        <input
                            v-model="keteranganBayar"
                            type="text"
                            placeholder="Misal: DP awal, Lunas di muka, dll"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition bg-white"
                        />
                    </div>
                </div>
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span v-if="checkedItems.size > 0" class="text-xs font-semibold text-amber-600 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full">
                    ...
                    <div class="relative mb-2">
                        ...
                        <input
                            ...
                            class="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition"
                        />
                    </div>
                    <div class="max-h-64 overflow-y-auto border border-gray-200 rounded-xl divide-y divide-gray-100">
                        <label
                            ...
                            :key="item.model"
                            class="flex items-center gap-3 px-3 py-2.5 hover:bg-amber-50/50 cursor-pointer transition-colors"
                            :class="checkedItems.has(item.model) ? 'bg-amber-50/60' : ''"
                        >
                            <input
                                type="checkbox"
                                :checked="checkedItems.has(item.model)"
                                @change="(e) => toggleItem(item, e)"
                                class="w-4 h-4 rounded border-gray-300 text-amber-500 focus:ring-amber-400 shrink-0"
                            />
                            ...
                            <template v-if="checkedItems.has(item.model)">
                                <input
                                    :value="formatRupiah(hargaPerItem[item.model] || 0)"
                                    type="text"
                                    disabled
                                    class="w-28 px-2 py-1.5 border border-amber-200 bg-gray-100 rounded-lg text-sm text-gray-500 text-right shrink-0 cursor-not-allowed"
                                    placeholder="harga/pcs"
                                />
                                <input
                                    v-model.number="pcsPerItem[item.model]"
                                    type="number"
                                    min="1"
                                    :max="item.sisa_stok"
                                    @click.stop
                                    class="w-20 px-2 py-1.5 border border-amber-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 bg-white text-right shrink-0"
                                    placeholder="pcs"
                                />
                            </template>
                        </label>
                        <div v-if="!filteredOptions.length" class="px-4 py-8 text-center text-sm text-gray-400">Tidak ada stok gudang yang tersedia</div>
                    </div>
                    <!-- Total -->
                    <div v-if="subtotal > 0" class="mt-3 flex flex-col gap-1">
                        <div class="flex items-center justify-between px-4 py-2 bg-amber-50 border border-amber-200 rounded-lg">
                            <span class="text-sm font-medium text-amber-700">Subtotal:</span>
                            <span class="text-base font-semibold text-amber-800">{{ formatRupiah(subtotal) }}</span>
                        </div>
                        <div class="flex items-center justify-between gap-3 px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg">
                            ...
                            <div class="flex items-center gap-2">
                                <input
                                    ...
                                    max="100"
                                    step="0.1"
                                    class="w-20 px-2 py-1 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 text-right"
                                    placeholder="0"
                                />
                ...
                <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                    ...
                    <button
                        ...
                        class="px-5 py-2.5 text-sm text-white bg-amber-500 hover:bg-amber-600 rounded-lg disabled:opacity-60 flex items-center gap-2"
                    >
        ...
        <Modal v-model="showEditModal" title="Edit Item" size="sm">
            <form @submit.prevent="submitEdit" class="space-y-4">
                ...
                <div>
                    ...
                    <input
                        v-model.number="editForm.harga_satuan"
                        type="number"
                        min="0"
                        required
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition bg-white"
                    />
                    <p v-if="editForm.pcs && editForm.harga_satuan" class="mt-1 text-xs text-amber-600">Total: {{ formatRupiah(editForm.pcs * editForm.harga_satuan) }}</p>
                </div>
                <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                    ...
                    <button type="submit" :disabled="editForm.processing" class="px-5 py-2.5 text-sm text-white bg-amber-500 hover:bg-amber-600 rounded-lg disabled:opacity-60 flex items-center gap-2">
        ...
        <ConfirmDialog v-model="showConfirm" title="Hapus Data?" message="Tindakan ini tidak dapat dibatalkan." :loading="deleteLoading" @confirm="doDelete" />
    </AdminLayout>
</template>
...
watch(searchQuery, (val) => {
    ...
    searchTimer = setTimeout(() => {
        router.get('/jual-gudang', { search: val || undefined }, { preserveState: true, replace: true });
    }, 350);
});
...
const submitPembayaran = () => {
    ...
    router.post(
        `/penjualan-pembayaran/${detailGroup.value.no_nota}`,
        {
            ...payForm.value,
            channel: 'gudang',
        },
...
const totalPcsSelected = computed(() => [...checkedItems.value].reduce((sum, key) => sum + (parseInt(pcsPerItem.value[key]) || 0), 0));
...
const submitEdit = () => {
    editForm.put(`/jual-gudang/${editRow.value.id}`, {
        onSuccess: () => {
...
const doDelete = () => {
    ...
    router.delete(`/jual-gudang/${deleteId.value}`, {
        onSuccess: () => {
            deleteId.value = null;
