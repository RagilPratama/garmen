<template>
    <AdminLayout title="Kas Garmen">
        <div class="space-y-5">
            <!-- Summary Cards -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="bg-gradient-to-br from-orange-400 to-orange-500 rounded-xl p-5 text-white shadow-md">
                    <p class="text-xs font-medium uppercase tracking-wide opacity-80">Cash</p>
                    <p class="text-2xl font-bold mt-1">{{ formatRupiah(Number(saldoKas?.saldo_cash || 0)) }}</p>
                </div>
                <div class="bg-gradient-to-br from-amber-400 to-amber-500 rounded-xl p-5 text-white shadow-md">
                    <p class="text-xs font-medium uppercase tracking-wide opacity-80">Transfer</p>
                    <p class="text-2xl font-bold mt-1">{{ formatRupiah(Number(saldoKas?.saldo_transfer || 0)) }}</p>
                </div>
                <div class="bg-gradient-to-br from-yellow-400 to-yellow-500 rounded-xl p-5 text-white shadow-md">
                    <p class="text-xs font-medium uppercase tracking-wide opacity-80">Debit</p>
                    <p class="text-2xl font-bold mt-1">{{ formatRupiah(Number(saldoKas?.saldo_debit || 0)) }}</p>
                </div>
            </div>

            <!-- Total Saldo -->
            <div class="bg-gradient-to-br from-orange-500 to-red-600 rounded-xl p-6 text-white shadow-lg">
                <p class="text-sm font-medium uppercase tracking-wide opacity-90">Total Saldo Kas Garmen</p>
                <p class="text-3xl font-bold mt-2">
                    {{ formatRupiah(Number(saldoKas?.saldo_cash || 0) + Number(saldoKas?.saldo_transfer || 0) + Number(saldoKas?.saldo_debit || 0)) }}
                </p>
            </div>

            <!-- Main Card -->
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="h-1 bg-gradient-to-r from-orange-400 via-amber-500 to-orange-400"></div>
                ...
                <div class="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-base font-semibold text-gray-800">Kas Garmen</h2>
                ...
                <div class="px-6 py-4 bg-gray-50/50 border-b border-gray-100">
                    <div class="flex flex-col md:flex-row gap-3">
                        <input
                            ...
                            class="px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition bg-white"
                        />
                        ...
                        <button @click="resetFilters" class="px-4 py-2.5 text-sm text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors whitespace-nowrap">
                            Reset Filter
                        </button>
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            <tr
                                ...
                                :key="kas.id"
                                class="border-b border-gray-50 hover:bg-orange-50/40 transition-colors group"
                                :class="index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'"
                            >
                                <td class="px-5 py-3.5">
                                    <span
                                        class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 text-xs font-medium text-gray-500 group-hover:bg-orange-100 group-hover:text-orange-700 transition-colors"
                                    >
                                ...
                                <td class="px-5 py-3.5">
                                    <span class="inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-full capitalize" :class="getMetodeClass(kas.metode_bayar)">
                                        {{ kas.metode_bayar }}
                                    </span>
                                </td>
                                ...
                                <td class="px-5 py-3.5 text-right font-semibold" :class="kas.jenis === 'masuk' ? 'text-green-600' : 'text-red-600'">
                                ...
                                <td class="px-5 py-3.5 text-center">
                                    <button @click="confirmDelete(kas.id)" class="text-red-400 hover:text-red-600 transition">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path
                                                stroke-linecap="round"
                                                stroke-linejoin="round"
                ...
                <!-- Pagination -->
                <div v-if="kasData?.last_page > 1" class="px-6 py-4 bg-gray-50/50 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <!-- CREATE MODAL -->
        <Modal v-model="showModal" title="Tambah Transaksi Kas Garmen">
            <form @submit.prevent="submit" class="space-y-4">
                <div>
                    ...
                    <input
                        ...
                        required
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition bg-white"
                    />
                </div>
                ...
                <div>
                    ...
                    <div class="flex gap-4 mt-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input v-model="form.jenis" type="radio" value="masuk" class="text-orange-500" />
                            <span class="text-sm text-gray-700">Masuk</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input v-model="form.jenis" type="radio" value="keluar" class="text-orange-500" />
                ...
                <div>
                    ...
                    <div class="flex gap-4 mt-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input v-model="form.metode_bayar" type="radio" value="cash" class="text-orange-500" />
                            <span class="text-sm text-gray-700">Cash</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input v-model="form.metode_bayar" type="radio" value="transfer" class="text-orange-500" />
                            <span class="text-sm text-gray-700">Transfer</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input v-model="form.metode_bayar" type="radio" value="debit" class="text-orange-500" />
                ...
                <div v-show="form.metode_bayar === 'transfer'" class="transition-all">
                    ...
                    >
                        ...
                        <option v-for="rek in rekenings || []" :key="rek.id" :value="rek.id">{{ rek.bank }} - {{ rek.nama }} {{ rek.nomor_rekening ? '(' + rek.nomor_rekening + ')' : '' }}</option>
                    </select>
                    <p v-if="!rekenings || rekenings.length === 0" class="text-xs text-gray-400 mt-1">
                        Belum ada rekening.
                        <a href="/rekening" class="text-blue-500 hover:underline">Tambah rekening</a>
                    </p>
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        required
                        placeholder="Contoh: Operasional Produksi, Gaji, dll"
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition bg-white"
                    />
                </div>
                ...
                <div>
                    ...
                    <textarea
                        ...
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition bg-white resize-none"
                    ></textarea>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                    ...
                    <button
                        type="submit"
                        :disabled="processing"
                        class="px-5 py-2.5 text-sm text-white bg-orange-500 hover:bg-orange-600 rounded-lg transition-colors disabled:opacity-60 flex items-center gap-2"
                    >
        ...
        <ConfirmDialog v-model="showConfirm" title="Hapus Transaksi?" message="Tindakan ini tidak dapat dibatalkan." :loading="deleteLoading" @confirm="doDelete" />

        <!-- TRANSFER MODAL -->
        <Modal v-model="showTransferModal" title="Transfer ke Gudang">
            <form @submit.prevent="submitTransfer" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="flex gap-4 mt-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input v-model="transferForm.metode_bayar" type="radio" value="cash" class="text-purple-500" />
                            <span class="text-sm text-gray-700">Cash</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input v-model="transferForm.metode_bayar" type="radio" value="transfer" class="text-purple-500" />
                            <span class="text-sm text-gray-700">Transfer</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input v-model="transferForm.metode_bayar" type="radio" value="debit" class="text-purple-500" />
                            <span class="text-sm text-gray-700">Debit</span>
                        </label>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                    <button type="button" @click="showTransferModal = false" class="px-5 py-2.5 text-sm text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                        Batal
                    </button>
                    <button
                        type="submit"
                    ...
                    >
                        <svg v-if="transferProcessing" class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                        </svg>
                        Transfer
...
const applyFilters = () => {
    router.get(route('kas-garmen.index'), filters, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const submit = () => {
    ...
    router.post(route('kas-garmen.store'), form, {
        onSuccess: () => {
...
const doDelete = () => {
    ...
    router.delete(route('kas-garmen.destroy', deleteId.value), {
        onFinish: () => {
...
const submitTransfer = () => {
    ...
    router.post(route('kas-garmen.transfer'), transferForm, {
        onSuccess: () => {
...
const resetTransferForm = () => {
    transferForm.entitas_penerima = 'gudang';
