<template>
  <DataTable
    ...
    :data="data"
    :columns="columns"
    base-path="/master-model"
    @open-create="openCreate"
    @open-edit="openEdit"
  >
    <template #modal>
      <Modal v-model="showModal" :title="editItem ? 'Edit Model' : 'Tambah Model'">
        <form @submit.prevent="submit" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Nama Model <span class="text-red-500">*</span></label>
            <input v-model="form.nama_model" type="text" required autofocus
            ...
            <p v-if="form.errors.nama_model" class="mt-1 text-xs text-red-500">{{ form.errors.nama_model }}</p>
          </div>
          <div>
...
const columns = [
  { key: 'nama_model', label: 'Nama Model' },
  { key: 'keterangan', label: 'Keterangan' },
]

const showModal = ref(false)
const editItem = ref(null)
const form = useForm({ nama_model: '', keterangan: '' })
...
const openEdit = (item) => {
  editItem.value = item
  form.nama_model  = item.nama_model  ?? ''
...
const submit = () => {
  if (editItem.value) {
    form.put(`/master-model/${editItem.value.id}`, {
      onSuccess: () => { showModal.value = false },
    })
  } else {
    form.post('/master-model', {
