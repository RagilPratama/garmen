        ...
        <div class="max-w-2xl">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-center gap-3 mb-6">
                    <Link href="/proses-cuci" class="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition">
...
const form = useForm({
    tanggal_kirim_cuci: props.item?.tanggal_kirim_cuci?.split('T')[0] ?? '',
    no_surat_jalan: props.item?.no_surat_jalan ?? '',
    po: props.item?.po ?? '',
    model: props.item?.model ?? '',
    pcs_kirim: props.item?.pcs_kirim ?? 0,
    tanggal_kembali_dari_cuci: props.item?.tanggal_kembali_dari_cuci?.split('T')[0] ?? '',
    pcs_kembali: props.item?.pcs_kembali ?? 0,
});
...
const submit = () => {
    isEdit.value ? form.put(`/proses-cuci/${props.item.id}`) : form.post('/proses-cuci');
};
</script>
