    ...
    <AdminLayout title="Proses Cuci">
                    ...
                    <tbody>
                        ...
                        >
                            <td class="px-5 py-3.5">
                                <span
                                    class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 text-xs font-medium text-gray-500 group-hover:bg-cyan-100 group-hover:text-cyan-700 transition-colors"
                                >
                            ...
                            <td class="px-5 py-3.5 text-center">
                                <span class="inline-flex items-center justify-center px-2.5 py-1 bg-cyan-50 text-cyan-700 text-xs font-semibold rounded-full">
                                    {{ group.total_pcs_kirim.toLocaleString('id-ID') }} pcs
                                </span>
                            </td>
            ...
            <div v-if="detailGroup" class="space-y-4">
                <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600 bg-gray-50 rounded-lg px-4 py-3">
                    <span>
                        ...
                        {{ formatDate(detailGroup.tanggal_kirim_cuci) }}
                    </span>
                    <span>
                        <span class="font-medium text-gray-700">No. SJ:</span>
                        <button v-if="detailGroup.no_surat_jalan" @click="printSuratJalan(detailGroup)" class="ml-1 text-blue-600 hover:underline font-semibold">
                    ...
                    <span>
                        <span class="font-medium text-gray-700">Total Pcs Kirim:</span>
                        {{ detailGroup.total_pcs_kirim }} pcs
                    </span>
                </div>
                <table class="w-full text-sm">
        ...
        <Modal v-model="showModal" title="Tambah Proses Cuci" size="lg">
            <div v-if="!poOptions.length" class="mb-4 flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
                ...
                Belum ada data dari
                <strong class="ml-1">Proses Jahit</strong>
                . Lengkapi kolom Pcs Hasil Jahit terlebih dahulu.
            </div>
            <form @submit.prevent="submit" class="space-y-4">
                <div>
                    ...
                    <SearchableSelect v-model="selectedPo" :options="uniquePos.map((p) => ({ value: p, label: p }))" placeholder="-- Pilih Nomor PO --" @update:modelValue="onPoChange" />
                </div>
                            ...
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <div class="flex items-center gap-3 shrink-0">
                                    ...
                                    <div class="flex items-center gap-1">
                                        ...
                                        <input
                                            v-model="kembaliPerModel[m.model]"
                                            type="number"
                                            min="0"
                ...
                <div v-if="selectedPo" class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input
                            v-model="form.tanggal_kirim_cuci"
                            type="date"
                            required
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-transparent transition bg-white"
                        />
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                type="text"
                                class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-transparent transition bg-white pr-14"
                ...
                <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                    ...
                    <button
                        type="submit"
                        :disabled="form.processing || !selectedPo"
                        class="px-5 py-2.5 text-sm text-white bg-cyan-500 hover:bg-cyan-600 rounded-lg disabled:opacity-60 flex items-center gap-2"
                    >
        ...
        <Modal v-model="showEditModal" title="Update Kembali Cuci" size="md">
            <form @submit.prevent="submitEdit" class="space-y-4">
                <div v-if="editRow" class="bg-gray-50 rounded-lg px-4 py-3 text-sm text-gray-600">
                    ...
                    {{ editRow.model }}
                    <span class="ml-4 font-medium text-gray-700">Pcs Kirim:</span>
                    {{ editRow.pcs_kirim }}
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input
                            v-model="editForm.tanggal_kembali_dari_cuci"
                            type="date"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-transparent transition bg-white"
                        />
                    </div>
...
const totalPcsKirim = computed(() => Object.values(pcsPerModel.value).reduce((sum, v) => sum + (parseInt(v) || 0), 0));
const onPoChange = () => {
    pcsPerModel.value = {};
    kembaliPerModel.value = {};
    modelsForPo.value.forEach((m) => {
        pcsPerModel.value[m.model] = '';
        kembaliPerModel.value[m.model] = '';
    });
};
const openCreate = () => {
    selectedPo.value = '';
...
const printSuratJalan = (group) => {
    ...
    // TABLE
    const tableRows = (group.models ?? []).map((m, i) => [i + 1, m.model ?? '—', Number(m.pcs_kirim ?? 0).toLocaleString('id-ID')]);
