        ...
        <div class="max-w-2xl">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-center gap-3 mb-6">
                    <Link href="/proses-finishing" class="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition">
...
const form = useForm({
    ...
    tanggal_proses: props.item?.tanggal_proses?.split('T')[0] ?? '',
    pcs: props.item?.pcs ?? 0,
    tanggal_selesai: props.item?.tanggal_selesai?.split('T')[0] ?? '',
    pcs_barang_jadi: props.item?.pcs_barang_jadi ?? 0,
    harga_satuan: props.item?.harga_satuan ?? 0,
});
...
const submit = () => {
    isEdit.value ? form.put(`/proses-finishing/${props.item.id}`) : form.post('/proses-finishing');
};
</script>
