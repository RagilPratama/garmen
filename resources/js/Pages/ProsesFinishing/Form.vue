...
const form = useForm({ po: props.item?.po ?? '', model: props.item?.model ?? '', tanggal_proses: props.item?.tanggal_proses?.split('T')[0] ?? '', pcs: props.item?.pcs ?? 0, tanggal_selesai: props.item?.tanggal_selesai?.split('T')[0] ?? '', pcs_barang_jadi: props.item?.pcs_barang_jadi ?? 0 })
