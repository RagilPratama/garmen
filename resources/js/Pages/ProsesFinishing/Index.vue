    ...
    <AdminLayout title="Proses Finishing & Packing">
                    ...
                    <tbody>
                        ...
                        >
                            <td class="px-5 py-3.5">
                                <span
                                    class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 text-xs font-medium text-gray-500 group-hover:bg-violet-100 group-hover:text-violet-700 transition-colors"
                                >
                            ...
                            <td class="px-5 py-3.5 text-center">
                                <span class="inline-flex items-center justify-center px-2.5 py-1 bg-violet-50 text-violet-700 text-xs font-semibold rounded-full">
                            ...
                            <td class="px-5 py-3.5 text-center">
                                <span class="inline-flex items-center justify-center px-2.5 py-1 bg-emerald-50 text-emerald-700 text-xs font-semibold rounded-full">
                                    {{ group.total_pcs_jadi.toLocaleString('id-ID') }} pcs
                                </span>
                            </td>
        ...
        <Modal v-model="showDetail" :title="'Detail PO: ' + (detailGroup?.po ?? '')" size="xl">
            <div v-if="detailGroup" class="space-y-4">
                <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600 bg-gray-50 rounded-lg px-4 py-3">
                    <span>
                        <span class="font-medium text-gray-700">Tgl Proses:</span>
                        {{ formatDate(detailGroup.tanggal_proses) }}
                    </span>
                    <span>
                    ...
                    <span>
                        <span class="font-medium text-gray-700">Total Barang Jadi:</span>
                        {{ detailGroup.total_pcs_jadi }} pcs
                    </span>
                </div>
                <table class="w-full text-sm">
        ...
        <Modal v-model="showModal" title="Tambah Proses Finishing" size="lg">
            <div v-if="!poOptions.length" class="mb-4 flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
                ...
                Belum ada data dari
                <strong class="ml-1">Proses Cuci</strong>
                . Lengkapi kolom Pcs Kembali terlebih dahulu.
            </div>
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                    ...
                    <button
                        ...
                        class="px-5 py-2.5 text-sm text-white bg-violet-500 hover:bg-violet-600 rounded-lg disabled:opacity-60 flex items-center gap-2"
                    >
        ...
        <Modal v-model="showEditModal" title="Update Selesai Finishing" size="md">
            <form @submit.prevent="submitEdit" class="space-y-4">
                <div v-if="editRow" class="bg-gray-50 rounded-lg px-4 py-3 text-sm text-gray-600">
                    ...
                    {{ editRow.model }}
                    <span class="ml-4 font-medium text-gray-700">Pcs:</span>
                    {{ editRow.pcs }}
                </div>
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <input
                                v-model="editForm.harga_satuan"
                                type="number"
                                min="0"
                                class="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 focus:border-transparent transition bg-white"
                                placeholder="0"
                            />
                        </div>
...
const totalPcs = computed(() => Object.values(pcsPerModel.value).reduce((sum, v) => sum + (parseInt(v) || 0), 0));
const onPoChange = () => {
    pcsPerModel.value = {};
    jadiPerModel.value = {};
    modelsForPo.value.forEach((m) => {
        pcsPerModel.value[m.model] = '';
        jadiPerModel.value[m.model] = '';
    });
};
const openCreate = () => {
    selectedPo.value = '';
