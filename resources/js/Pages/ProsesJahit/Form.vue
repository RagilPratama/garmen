        ...
        <div class="max-w-2xl">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-center gap-3 mb-6">
                    <Link href="/proses-jahit" class="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition">
...
const form = useForm({
    tanggal_jahit: props.item?.tanggal_jahit?.split('T')[0] ?? '',
    po: props.item?.po ?? '',
    model: props.item?.model ?? '',
    pcs_potongan: props.item?.pcs_potongan ?? 0,
    tanggal_selesai_jahit: props.item?.tanggal_selesai_jahit?.split('T')[0] ?? '',
    pcs_hasil_jahit: props.item?.pcs_hasil_jahit ?? 0,
});
...
const submit = () => {
    isEdit.value ? form.put(`/proses-jahit/${props.item.id}`) : form.post('/proses-jahit');
};
</script>
