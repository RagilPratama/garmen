    ...
    <AdminLayout title="Proses Jahit">
                        ...
                        >
                            ...
                            <td class="px-5 py-3.5 text-center">
                                <span class="inline-flex items-center justify-center px-2.5 py-1 bg-emerald-50 text-emerald-700 text-xs font-semibold rounded-full">
                                    {{ group.total_pcs_hasil_jahit.toLocaleString('id-ID') }} pcs
                                </span>
                            </td>
            ...
            <div v-if="detailGroup" class="space-y-4">
                <div class="flex items-center gap-4 text-sm text-gray-600 bg-gray-50 rounded-lg px-4 py-3">
                    <span>
                        ...
                        {{ formatDate(detailGroup.tanggal_jahit) }}
                    </span>
                    <span>
                    ...
                    <span>
                        ...
                        {{ detailGroup.total_pcs_hasil_jahit.toLocaleString('id-ID') }} pcs
                    </span>
                </div>
                <table class="w-full text-sm">
        ...
        <Modal v-model="showModal" title="Tambah Proses Jahit" size="lg">
            <div v-if="!poOptions.length" class="mb-4 flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
                ...
                Belum ada data dari
                <strong class="ml-1">Proses Potong</strong>
                . Lengkapi kolom Hasil Potongan terlebih dahulu.
            </div>
            <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div class="divide-y divide-amber-100">
                        <div v-for="m in modelsForPo" :key="m.model" class="px-3 py-2.5">
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <span class="text-xs font-semibold bg-white border border-amber-200 text-amber-700 px-2 py-0.5 rounded-full shrink-0">{{ m.max_pcs }} pcs</span>
                                <div class="flex items-center gap-1.5 shrink-0">
                                    <label class="text-xs text-gray-500">Hasil jahit:</label>
                                    <input
                                        v-model="hasilPerModel[m.model]"
                                        type="number"
                                        min="0"
                ...
                <div v-if="selectedPo" class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input
                            v-model="form.tanggal_selesai_jahit"
                            type="date"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition bg-white"
                        />
                    </div>
                ...
                <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                    ...
                    <button
                        ...
                        class="px-5 py-2.5 text-sm text-white bg-orange-500 hover:bg-orange-600 rounded-lg disabled:opacity-60 flex items-center gap-2"
                    >
            ...
            <form @submit.prevent="submitEdit" class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input
                            v-model="editForm.tanggal_jahit"
                            type="date"
                            required
                    ...
                    <div>
                        ...
                        <input
                            v-model="editForm.pcs_hasil_jahit"
                            type="number"
                            min="0"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition bg-white"
                            placeholder="0"
                    ...
                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Selesai Jahit</label>
                        <input
                            v-model="editForm.tanggal_selesai_jahit"
                            type="date"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition bg-white"
                        />
                    </div>
...
const totalPcsPotongan = computed(() => Object.values(hasilPerModel.value).reduce((sum, v) => sum + (parseInt(v) || 0), 0));
