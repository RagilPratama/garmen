<template>
  <DataTable title="Jual dari Toko" :data="data" :columns="columns" base-path="/proses-jual"
    @open-create="openCreate" @open-edit="openEdit">
    <template #modal>
      <Modal v-model="showModal" :title="editItem ? 'Edit Jual Toko' : 'Tambah Jual Toko'" size="lg">
        ...
        <form @submit.prevent="submit" class="space-y-4">
          <div>
            ...
            <SearchableSelect
              v-model="form.model"
              :options="stokOptions.map(o => ({ value: o.model, label: o.model + ' — stok: ' + o.sisa_stok + ' pcs' }))"
              placeholder="-- Pilih model dari stok toko --"
              @change="onSelect"
            />
          </div>
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="col-span-2">
              ...
              <input v-model="form.buyer" type="text" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition bg-white" placeholder="Nama toko / pembeli"/>
...
const submit = () => {
  if (editItem.value) {
    form.put(`/proses-jual/${editItem.value.id}`, { onSuccess: () => { showModal.value = false } })
  } else {
    form.post('/proses-jual', { onSuccess: () => { showModal.value = false; form.reset(); form.status = 'pending' } })
  }
}
</script>
