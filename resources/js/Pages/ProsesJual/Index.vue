    ...
    <Modal v-model="showDetail" :title="'Detail Nota: ' + (detailGroup?.no_nota ?? '—')" size="xl">
      <div v-if="detailGroup" class="space-y-4">
        <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600 bg-gray-50 rounded-lg px-4 py-3">
          ...
          <span><span class="font-medium text-gray-700">Total:</span> <span class="font-semibold text-violet-700">{{ formatRupiah(detailGroup.total_harga) }}</span></span>
        </div>
        <!-- Status update -->
        <div class="flex items-center gap-3">
          ...
          <div class="flex gap-2">
            <button v-for="s in ['pending', 'lunas', 'batal']" :key="s"
              ...
              :class="detailGroup.status === s ? statusClass(s) + ' ring-2 ring-offset-1 ring-violet-400' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'"
        ...
        <table class="w-full text-sm">
          ...
          <tfoot class="bg-gray-50">
            <tr>
              <td class="px-4 py-2.5 text-xs font-semibold text-gray-500">Total</td>
              <td class="px-4 py-2.5 text-center font-bold text-violet-700">{{ detailGroup.total_pcs }}</td>
              <td></td>
...
const updateNotaStatus = (noNota, status) => {
  ...
  router.put('/proses-jual-nota-status', { no_nota: noNota, status }, {
    onSuccess: () => {
