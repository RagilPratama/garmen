    ...
    <AdminLayout title="Rincian Bahan Masuk & Keluar">
        <div class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div v-for="s in summary" :key="s.kode_bahan" class="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4 flex items-center gap-4">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" :class="tab === 'masuk' ? 'bg-emerald-50' : 'bg-rose-50'">
                        <svg class="w-5 h-5" :class="tab === 'masuk' ? 'text-emerald-500' : 'text-rose-500'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div v-if="!summary.length" class="col-span-full bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-8 text-center text-sm text-gray-400">Belum ada data</div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-xl flex items-center justify-center" :class="tab === 'masuk' ? 'bg-emerald-50' : 'bg-rose-50'">
                            <svg class="w-5 h-5" :class="tab === 'masuk' ? 'text-emerald-500' : 'text-rose-500'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <div class="relative">
                            ...
                            <input
                                v-model="searchQuery"
                                type="text"
                                placeholder="Cari kode bahan, nota..."
                ...
                <div v-else class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            <tr v-for="(row, i) in rows?.data" :key="row.id" class="border-b border-gray-50 hover:bg-rose-50/30 transition-colors" :class="i % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'">
                                <td class="px-5 py-3">
                                    <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 text-xs font-medium text-gray-500">
                                        {{ (rows.current_page - 1) * rows.per_page + i + 1 }}
                                    </span>
                ...
                <div v-if="rows?.last_page > 1" class="px-6 py-4 bg-gray-50/50 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
                    <p class="text-xs text-gray-500">
                        Menampilkan
                        <span class="font-semibold text-gray-700">{{ rows.from }}–{{ rows.to }}</span>
                        dari
                        <span class="font-semibold text-gray-700">{{ rows.total }}</span>
                        baris
                    </p>
