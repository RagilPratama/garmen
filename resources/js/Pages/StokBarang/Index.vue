        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                <div class="border-b border-gray-100 flex">
                    ...
                    >
                        {{ tab.label }}
                        <span :class="['ml-2 px-1.5 py-0.5 rounded-full text-xs font-semibold', activeTab === tab.key ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-500']">
                            ...
                            <tr v-for="row in filteredToko" :key="row.model" class="hover:bg-gray-50/60 transition-colors">
                                ...
                                <td class="px-6 py-3.5 text-center">
                                    <button
                                        ...
                                        class="inline-flex items-center px-3 py-1.5 rounded-md text-xs font-medium bg-amber-100 text-amber-700 hover:bg-amber-200 transition-colors"
                                    >
                                        Edit
                                    </button>
                                </td>
            ...
            <Modal v-model="editModalOpen" title="Edit Stok Toko" size="lg">
                <form @submit.prevent="submitEdit" class="space-y-4">
                    ...
                    <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                        <button type="button" @click="closeEditModal" class="px-5 py-2.5 text-sm text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">Batal</button>
                        <button
                            type="submit"
            ...
            <Modal v-model="editKantorModalOpen" title="Edit Stok Kantor" size="lg">
                <form @submit.prevent="submitEditKantor" class="space-y-4">
                    <div>
                        <p class="text-sm text-gray-600">
                            Model:
                            <span class="font-semibold">{{ editKantorModel }}</span>
                        </p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Jumlah Sisa Kantor</label>
                        <input
                            v-model.number="editKantorForm.sisa_kantor"
                            type="number"
                            min="0"
                    ...
                    <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                        <button type="button" @click="closeEditKantorModal" class="px-5 py-2.5 text-sm text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                            Batal
                        </button>
                        <button
                            type="submit"
                            :disabled="editKantorForm.processing"
...
const uniqueModels = computed(() => new Set([...props.stokKantor.map((r) => r.model), ...props.stokToko.map((r) => r.model)]).size);
</script>
