<template>
    <AdminLayout title="Buat Surat Jalan">
        <div class="space-y-6">
            <!-- Header -->
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-semibold text-gray-800">Buat Surat Jalan Baru</h1>
                    <p class="text-sm text-gray-500 mt-0.5">Buat surat jalan dulu, lalu scan barcode di halaman detail</p>
                </div>
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    >
                        {{ saving ? 'Menyimpan...' : 'Buat Surat Jalan & Mulai Scan' }}
                    </button>
                </form>
            </div>
        </div>
...
const submit = () => {
    saving.value = true;
    router.post('/surat-jalan-garmen', form.value, {
        onFinish: () => {
            saving.value = false;
        },
    });
